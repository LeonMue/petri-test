...
public class NestedEmpty {
    ...
    public byte[] serialize() {
        ...
        try (final DataOutputStream yyDataOutput = new DataOutputStream(yySerializedFields)) {
            if (this.hasEmpty()) {
                yyBitVector[0] |= 1;
                final byte[] serialized = empty.serialize();
    ...
    public static NestedEmpty deserialize(byte[] serialized) throws org.dhbw.ka.ml.petrilib.serializing.ParseException {
        final NestedEmpty yyResultValue = new NestedEmpty();
        try (final org.dhbw.ka.ml.petrilib.io.PetriReader yyIn = new org.dhbw.ka.ml.petrilib.io.PetriReader(serialized)) {
            ...
            if ((serialized[0] & 1) != 0) {
                ...
                yyResultValue.setEmpty(deserialized);
            }
            if ((serialized[0] & 2) != 0) {
    ...
    static NestedEmpty yyDeserialize(org.dhbw.ka.ml.petrilib.io.PetriReader petriReader, int length) throws IOException {
        final NestedEmpty yyResultValue = new NestedEmpty();
